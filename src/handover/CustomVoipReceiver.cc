...
Define_Module(inet::CustomVoipReceiver);

simsignal_t CustomVoipReceiver::voipPacketLossRateSignal = registerSignal("voipPacketLossRate");
simsignal_t CustomVoipReceiver::voipPacketDelaySignal = registerSignal("voipPacketDelay");
simsignal_t CustomVoipReceiver::voipPlayoutDelaySignal = registerSignal("voipPlayoutDelay");
simsignal_t CustomVoipReceiver::voipPlayoutLossRateSignal = registerSignal("voipPlayoutLossRate");
simsignal_t CustomVoipReceiver::voipMosRateSignal = registerSignal("voipMosRate");
simsignal_t CustomVoipReceiver::voipTaildropLossRateSignal = registerSignal("voipTaildropLossRate");

void CustomVoipReceiver::TalkspurtInfo::startTalkspurt(const CustomVoipPacket *pk)
{
    status = ACTIVE;
...
bool CustomVoipReceiver::TalkspurtInfo::checkPacket(const CustomVoipPacket *pk)
{
...
void CustomVoipReceiver::TalkspurtInfo::addPacket(const CustomVoipPacket *pk)
{
...
CustomVoipReceiver::CustomVoipReceiver()
{
}
...
void CustomVoipReceiver::initialize(int stage)
{
    ...
    if (stage == INITSTAGE_LOCAL) {
        ...
        EV_INFO << "CustomVoipReceiver::initialize - binding to port: local:" << port << endl;
        if (port != -1) {
...
void CustomVoipReceiver::startTalkspurt(Packet *packet)
{
    const auto& voice = packet->peekAtFront<CustomVoipPacket>();
...
void CustomVoipReceiver::handleMessage(cMessage *msg)
{
...
void CustomVoipReceiver::socketDataArrived(UdpSocket *socket, Packet *packet)
{
...
void CustomVoipReceiver::socketErrorArrived(UdpSocket *socket, Indication *indication)
{
    EV_WARN << "Unknown message '" << indication->getName() << "', kind = " << indication->getKind() << ", discarding it." << endl;
    delete indication;
}

void CustomVoipReceiver::evaluateTalkspurt(bool finish)
{
...
double CustomVoipReceiver::eModel(double delay, double lossRate)
{
...
void CustomVoipReceiver::finish()
{
